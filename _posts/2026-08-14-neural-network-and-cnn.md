...
내부구조가 이렇게 복잡하구나 하고 넘어가도 충분하다.

중요한건 RNN에서는 Hidden State만 있었고 장기상태, 단기상태가 존재한다.

내부구조는 세가지 Gate가 있다는 점.

GRU는 내부 구조가 어떤 연산을 수행하고 연결보다는, 이 부분은 Reset Gate, Update Gate 두개로 이루어져 있다. LSTM보다 간략하다.
